...
class Token(BaseModel):
    ...
    # Authentifizierungsinformationen (Kern)
    st: str  # Session Token (__Secure-next-auth.session-token)
    at: Optional[str] = None  # Access Token (vonSTKonvertierenUndKommen)
    at_expires: Optional[datetime] = None  # ATAblaufzeit
    ...
    # VideoFXBesondershatFeld
    credits: int = 0  # Verbleibendcredits
    user_paygate_tier: Optional[str] = None  # PAYGATE_TIER_ONE

    # Projektverwaltung
    current_project_id: Optional[str] = None  # wennvorVerwendenProjektUUID
    current_project_name: Optional[str] = None  # Projektname

    # Funktionsschalter
    image_enabled: bool = True
    video_enabled: bool = True

    # Parallelitaetsbegrenzung
    image_concurrency: int = -1  # -1TabelleAnzeigenkeinBegrenzung
    video_concurrency: int = -1  # -1TabelleAnzeigenkeinBegrenzung

    # Captcha-Proxy(token Level, kannUeberschreibenalleBrowser-Captcha-LoesungProxy)
    captcha_proxy_url: Optional[str] = None

    # 429Deaktivierenschliessen
    ban_reason: Optional[str] = None  # Deaktivierungsgrund: "429_rate_limit" Oder None
    banned_at: Optional[datetime] = None  # Deaktivierungszeitpunkt
...
class Project(BaseModel):
    ...
    id: Optional[int] = None
    project_id: str  # VideoFXProjektUUID
    token_id: int  # schliessenVerbindenToken ID
    project_name: str  # Projektname
    tool_name: str = "PINHOLE"  # WerkzeugNameBezeichnung,FestBestimmenfuerPINHOLE
    is_active: bool = True
    created_at: Optional[datetime] = None
...
class TokenStats(BaseModel):
    ...
    # VerbindenFortsetzenFehlerzaehlung (verwendenFuerAutomatischDeaktivierenBestimmen)
...
class Task(BaseModel):
    ...
    id: Optional[int] = None
    task_id: str  # Flow APIZurueckgebenoperation name
    token_id: int
    model: str
    prompt: str
    ...
    scene_id: Optional[str] = None  # Flow APIsceneId
    created_at: Optional[datetime] = None
...
class ProxyConfig(BaseModel):
    ...
    id: int = 1
    enabled: bool = False  # Anfrage-ProxySchalter
    proxy_url: Optional[str] = None  # Anfrage-ProxyAdresse
    media_proxy_enabled: bool = False  # Bild-Upload/Download-ProxySchalter
    media_proxy_url: Optional[str] = None  # Bild-Upload/Download-ProxyAdresse
...
class CaptchaConfig(BaseModel):
    ...
    browser_proxy_enabled: bool = False  # Browser-Captcha-LoesungistObAktivierenProxy
    browser_proxy_url: Optional[str] = None  # Browser-Captcha-LoesungProxyURL
    browser_count: int = 1  # Browser-Captcha-LoesungInstanzAnzahlMenge
    created_at: Optional[datetime] = None
    updated_at: Optional[datetime] = None
...
class PluginConfig(BaseModel):
    ...
    id: int = 1
    connection_token: str = ""  # PluginKomponenteVerbindungtoken
    auto_enable_on_update: bool = True  # AktualisierentokenbeiAutomatischAktivieren(StandardoeffnenStarten)
    created_at: Optional[datetime] = None
    updated_at: Optional[datetime] = None
