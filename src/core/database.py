...
class Database:
        ...
        async with self._connect(write=True) as db:
            ...
            if await self._table_exists(db, "tokens"):
                columns_to_add = [
                    ...
                    ("captcha_proxy_url", "TEXT"),  # tokenLevelCaptcha-Proxy
                    ("ban_reason", "TEXT"),  # Deaktivierungsgrund
                    ("banned_at", "TIMESTAMP"),  # Deaktivierungszeitpunkt
                ]
            ...
            if await self._table_exists(db, "token_stats"):
                stats_columns_to_add = [
                    ...
                    ("today_date", "DATE"),
                    ("consecutive_error_count", "INTEGER DEFAULT 0"),  # 🆕 VerbindenFortsetzenFehlerzaehlung
                ]
            ...
            if await self._table_exists(db, "plugin_config"):
                plugin_columns_to_add = [
                    ("auto_enable_on_update", "BOOLEAN DEFAULT 1"),  # StandardoeffnenStarten
                ]
    ...
    async def init_db(self):
        ...
        async with self._connect(write=True) as db:
            ...
            # Request logs query indexes (Listenach created_at Reihenfolge / token )
