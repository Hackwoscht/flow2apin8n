...
def _extract_error_summary(payload: Any) -> str:
    """vonAntwortBodyinExtrahierenBenutzerkannlesenFehlerzusammenfassung。"""
    if payload is None:
        return ""
...
def _guess_client_hints_from_user_agent(user_agent: str) -> Dict[str, str]:
    """BasisBasierend auf UA ErgaenzenalleHaeufigSehen sec-ch-* Header。"""
...
def _guess_impersonate_from_user_agent(user_agent: str) -> str:
    """von UA AuswahlVerfuegbar curl_cffi Browser-FingerabdruckVersion。"""
    ua = (user_agent or "").strip()
    major_match = re.search(r"(?:Chrome|Chromium|Edg|EdgA|EdgiOS)/(\d+)", ua)
    if not major_match:
        return "chrome120"
...
def _normalize_http_base_url(base_url: str) -> str:
    ...
    if not normalized:
        raise RuntimeError("Remote-CaptchaDienstAdressenichtKonfiguration")

    parsed = urlparse(normalized)
    if parsed.scheme not in {"http", "https"} or not parsed.netloc:
        raise RuntimeError("Remote-CaptchaDienstAdresseFormatFehler, Mussist http(s)://host[:port]")
...
def _get_remote_browser_client_config() -> tuple[str, str, int]:
    ...
    if not api_key:
        raise RuntimeError("Remote-CaptchaDienst API Key nichtKonfiguration")
...
async def _sync_json_http_request(
    ...
    try:
        ...
        raise RuntimeError(f"Remote-CaptchaDienstAnfrageFehlgeschlagen: {e}") from e
...
async def _resolve_score_test_verify_proxy(
    ...
    """
    Auswahl score-test  verify Anfrage-Proxy, BevorzugtmitBrowser-Captcha-LoesungProxyKonsistent。
    Zurueckgeben: (proxies, used, source, proxy_url)
    """
    # Browser-Captcha-LoesungModusBevorzugtVerwenden browser_proxy, Sicherstellenmitabrufen token AusgangKonsistent
    ...
    # ZurueckZurueckAnfrage-ProxyKonfiguration
    try:
        if proxy_manager:
...
async def _solve_recaptcha_with_api_service(
    ...
    """VerwendenwennvorKonfigurationNr.Captcha-DienstAbrufen token。"""
    if method == "yescaptcha":
        ...
        raise RuntimeError(f"nichtUnterstuetztCaptcha-Methode: {method}")

    if not client_key:
        raise RuntimeError(f"{method} API Key nichtKonfiguration")
    ...
    async with AsyncSession() as session:
        ...
        if not task_id:
            ...
            raise RuntimeError(f"{method} createTask Fehlgeschlagen: {error_desc}")
        ...
        for _ in range(40):
            ...
            if poll_json.get("status") == "ready":
                ...
                if token:
                    return token
                raise RuntimeError(f"{method} ZurueckgebenErgebnisErgebnisFehlenwenige token: {poll_json}")
            ...
            if poll_json.get("errorId") not in (None, 0):
                ...
                raise RuntimeError(f"{method} getTaskResult Fehlgeschlagen: {error_desc}")

            await asyncio.sleep(3)

    raise RuntimeError(f"{method} Abrufen token Timeout")
...
async def _score_test_with_remote_browser_service(
    ...
    """AufrufenverwendenRemoteProzesshatHeaderCaptcha-DienstAusfuehrenSeiteinnerhalbCaptcha-Loesung+AnzahlValidierung。"""
    ...
    if status_code >= 400:
        ...
        raise RuntimeError(f"Remote-CaptchaDienstAnfrageFehlgeschlagen (HTTP {status_code}): {detail or 'nichtWissenFehler'}")

    if not isinstance(response_payload, dict):
        raise RuntimeError("Remote-CaptchaDienstZurueckgebenFormatFehler")
...
class AddTokenRequest(BaseModel):
    st: str
    project_id: Optional[str] = None  # BenutzerOptionalEingabeproject_id
...
class UpdateTokenRequest(BaseModel):
    st: str  # Session Token (MussFuellen, verwendenFuerAktualisierenAT)
    project_id: Optional[str] = None  # BenutzerOptionalEingabeproject_id
    project_name: Optional[str] = None
    remark: Optional[str] = None
    captcha_proxy_url: Optional[str] = None
...
async def admin_logout(token: str = Depends(verify_admin_token)):
    ...
    return {"success": True, "message": "AbmeldenAnmeldenErfolgreich"}
...
async def change_password(
    ...
    if not AuthManager.verify_admin(admin_config.username, request.old_password):
        raise HTTPException(status_code=400, detail="altPasswortFehler")
    ...
    return {"success": True, "message": "PasswortAendernErfolgreich,BitteErneutneuAnmelden"}
...
async def get_tokens(token: str = Depends(verify_admin_token)):
    ...
    return [{
        ...
        "at": row.get("at"),  # Access Token for editing (vonSTKonvertierenUndKommen)
        "at_expires": to_iso(row.get("at_expires")) if row.get("at_expires") else None,  # 🆕 ATAblaufzeit
        "token": row.get("at"),  # KompatibelvorEnde token.token ZugriffFormat
        "email": row.get("email"),
        "name": row.get("name"),
        ...
        "credits": row.get("credits"),  # 🆕 Guthaben
        "user_paygate_tier": row.get("user_paygate_tier"),
        "current_project_id": row.get("current_project_id"),  # 🆕 ProjektID
        "current_project_name": row.get("current_project_name"),  # 🆕 Projektname
    ...
    } for row in token_rows]  # DirektZurueckgebenAnzahlGruppe,KompatibelvorEnde
...
async def add_token(
    ...
    try:
        new_token = await token_manager.add_token(
            st=request.st,
            project_id=request.project_id,  # 🆕 UnterstuetztBenutzerAngegebenproject_id
        ...
        # AktualisierenParallelitaetsbegrenzung, VermeidenMussNeustartDienst
        ...
        return {
            "success": True,
            "message": "TokenHinzufuegenErfolgreich",
            "token": {
                "id": new_token.id,
        ...
        raise HTTPException(status_code=500, detail=f"HinzufuegenTokenFehlgeschlagen: {str(e)}")
...
async def update_token(
    ...
    """Update token - VerwendenSTAuto-AktualisierungAT"""
    try:
        # ZuerstSTAT
        ...
        # Aktualisierentoken (EnthaeltAT, ST, ATAblaufzeit, project_idUndproject_name)
        await token_manager.update_token(
            token_id=token_id,
            st=request.st,
            at=at,
            at_expires=at_expires,  # 🆕 AktualisierenATAblaufzeit
        ...
        # AktualisierenParallelitaetsbegrenzung, SicherstellenVerwaltungKonsoleAendernSofortSofortGenerierenWirksam
        if concurrency_manager:
        ...
        return {"success": True, "message": "TokenAktualisierenErfolgreich"}
    except Exception as e:
...
async def delete_token(
    ...
    try:
        ...
        return {"success": True, "message": "TokenLoeschenErfolgreich"}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
async def enable_token(
    ...
    return {"success": True, "message": "TokenbereitsAktivieren"}
...
async def disable_token(
    ...
    return {"success": True, "message": "TokenbereitsDeaktivieren"}
...
async def refresh_credits(
    ...
    """AktualisierenTokenGuthaben 🆕"""
    try:
        credits = await token_manager.refresh_credits(token_id)
        return {
            "success": True,
            "message": "Guthaben-AktualisierungErfolgreich",
            "credits": credits
        }
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"AktualisierenGuthabenFehlgeschlagen: {str(e)}")
...
async def refresh_at(
    ...
    """ManuellAktualisierenTokenAT (VerwendenSTKonvertieren) 🆕
    
    wieErgebnis AT AktualisierenFehlgeschlagenundVerarbeitenFuer personal Modus, wirdAutomatischVersuchenversuchenDurchBrowserAktualisieren ST
    """
    from ..core.logger import debug_logger
    from ..core.config import config
    
    debug_logger.log_info(f"[API] ManuellAktualisieren AT Anfrage: token_id={token_id}, captcha_method={config.captcha_method}")
    
    try:
        # Aufrufenverwendentoken_managerinnerhalbTeilAktualisierenMethode(Enthaelt ST Auto-AktualisierungLogik)
        success = await token_manager._refresh_at(token_id)

        if success:
            # AbrufenAktualisierennachtokenInformationen
            updated_token = await token_manager.get_token(token_id)
            
            message = "ATAktualisierenErfolgreich"
            if config.captcha_method == "personal":
                message += "(UnterstuetztSTAuto-Aktualisierung)"
            
            debug_logger.log_info(f"[API] AT AktualisierenErfolgreich: token_id={token_id}")
            ...
            debug_logger.log_error(f"[API] AT AktualisierenFehlgeschlagen: token_id={token_id}")
            
            error_detail = "ATAktualisierenFehlgeschlagen"
            if config.captcha_method != "personal":
                error_detail += f"(wennvorCaptcha-LoesungModus: {config.captcha_method}, STAuto-AktualisierungNurin personal ModusunterVerfuegbar)"
            
            raise HTTPException(status_code=500, detail=error_detail)
    except HTTPException:
        raise
    except Exception as e:
        debug_logger.log_error(f"[API] AktualisierenATAusnahme: {str(e)}")
        raise HTTPException(status_code=500, detail=f"AktualisierenATFehlgeschlagen: {str(e)}")
...
async def st_to_at(
    ...
    """Convert Session Token to Access Token (NurKonvertieren,nichtHinzufuegenaufDatenbank)"""
    try:
        result = await token_manager.flow_client.st_to_at(request.st)
        return {
            "success": True,
...
async def import_tokens(
    ...
    """BatchMengeImportierenToken"""
    from datetime import datetime, timezone

    added = 0
    updated = 0
    errors = []
    # mitHistorischGeschichteLogikKonsistent: nach created_at DESC ErgebnisErgebnisin, BevorzugtTreffeninGleichE-MailPostfach"Am meistenneueinEintrag"
    existing_by_email = {}
    ...
    for idx, item in enumerate(request.tokens):
        try:
            ...
            if not st:
                errors.append(f"Nr.{idx+1}Element: Fehlenwenige session_token")
                continue

            # Verwenden ST  AT AbrufenBenutzerInformationen
            try:
                ...
                if not email:
                    errors.append(f"Nr.{idx+1}Element: keinMethodeAbrufenE-MailPostfachInformationen")
                    continue
                ...
                if expires:
                    try:
                        at_expires = datetime.fromisoformat(expires.replace('Z', '+00:00'))
                        # BestimmenistObAbgelaufen
                        now = datetime.now(timezone.utc)
                        is_expired = at_expires <= now
                    except:
                        pass

                # VerwendenE-MailPostfachPruefenistObbereitsspeichernin
                existing = existing_by_email.get(email)

                if existing:
                    # AktualisierenAktuellhatToken
                    await token_manager.update_token(
                    ...
                    # wieErgebnisAbgelaufendannDeaktivieren
                    if is_expired:
                ...
                errors.append(f"Nr.{idx+1}Element: {str(e)}")
    ...
    return {
        ...
        "message": f"ImportierenAbgeschlossen: neuHinzufuegen {added} Stueck, Aktualisieren {updated} Stueck" + (f", {len(errors)} StueckFehlgeschlagen" if errors else "")
    }
...
async def update_proxy_config_alias(
    ...
    return {"success": True, "message": "Proxy-KonfigurationAktualisierenErfolgreich"}
...
async def test_proxy_connectivity(
    ...
    """TestProxyistObkannZugriffZielZielSeite(Standard https://labs.google/)"""
    ...
    try:
        ...
        return {
            "success": ok,
            "message": "ProxyVerfuegbar" if ok else f"ProxykannVerbindenDurch, aberZielZielZurueckgebenStatusCode {status_code}",
            "test_url": test_url,
            "final_url": final_url,
        ...
        return {
            "success": False,
            "message": f"ProxyTestFehlgeschlagen: {str(e)}",
            "test_url": test_url,
            "elapsed_ms": elapsed_ms
        }
...
async def update_generation_config(
    ...
    return {"success": True, "message": "GenerierungKonfigurationAktualisierenErfolgreich"}
...
async def update_call_logic_config(
    ...
    return {
        "success": True,
        "message": "TokenRotationsmodusSpeichernErfolgreich",
        "config": {
            "call_mode": call_mode,
            "polling_mode_enabled": call_mode == "polling",
        }
...
async def get_log_detail(
    ...
    if not log:
        raise HTTPException(status_code=404, detail="Protokollnichtspeichernin")
...
async def clear_logs(token: str = Depends(verify_admin_token)):
    ...
    try:
        ...
        return {"success": True, "message": "allehatProtokollbereitsBereinigenleer"}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
async def update_admin_config(
    ...
    return {"success": True, "message": "KonfigurationAktualisierenErfolgreich"}
...
async def update_api_key(
    ...
    return {"success": True, "message": "API KeyAktualisierenErfolgreich"}
...
async def get_token_refresh_config(token: str = Depends(verify_admin_token)):
    """Get AT auto refresh configuration (StandardAktivieren)"""
    return {
        "success": True,
        "config": {
            "at_auto_refresh_enabled": True  # Flow2APIStandardAktivierenATAuto-Aktualisierung
        }
    }
...
async def update_token_refresh_enabled(
    ...
    """Update AT auto refresh enabled (Flow2APIFestBestimmenAktivieren,dieseSchnittstelleNurverwendenFuervorEndeKompatibel)"""
    return {
        "success": True,
        "message": "Flow2APIATAuto-AktualisierungStandardAktivierenundkeinMethodeSchliessen"
    }
...
async def update_cache_enabled(
    ...
    return {"success": True, "message": f"Cachebereits{'Aktivieren' if enabled else 'Deaktivieren'}"}
...
async def update_cache_config_full(
    ...
    if timeout is not None:
        try:
            ...
            raise HTTPException(status_code=400, detail="CacheTimeoutbeiZwischenMussfuerGesamtAnzahl")
        if timeout < 0:
            raise HTTPException(status_code=400, detail="CacheTimeoutbeiZwischennichtkannkleinFuer 0")
    ...
    return {"success": True, "message": "CacheKonfigurationAktualisierenErfolgreich"}
...
async def update_cache_base_url(
    ...
    return {"success": True, "message": "CacheBase URLAktualisierenErfolgreich"}
...
async def update_captcha_config(
    ...
    # ValidierenBrowserProxyURLFormat
    ...
    try:
        ...
        return {"success": False, "message": "Remote-CaptchaTimeoutbeiZwischenMussistGesamtAnzahlSekunden"}

    if captcha_method == "remote_browser":
        if not (remote_browser_base_url or "").strip():
            return {"success": False, "message": "remote_browser ModusbenoetigenbrauchenKonfigurationRemote-CaptchaDienstAdresse"}
        if not (remote_browser_api_key or "").strip():
            return {"success": False, "message": "remote_browser ModusbenoetigenbrauchenKonfigurationRemote-CaptchaDienst API Key"}
    ...
    # wieErgebnisVerwenden browser Captcha-Loesung, ErneutLadenBrowserAnzahlMengeKonfiguration
    ...
    return {"success": True, "message": "CaptchaKonfigurationAktualisierenErfolgreich"}
...
async def test_captcha_score(
    ...
    """VerwendenwennvorCaptcha-MethodeAbrufen token, undEinreichenUebergebenauf antcpt ValidierungAnzahl。"""
    ...
    try:
        ...
        if captcha_method == "browser":
            ...
            if isinstance(score_payload, dict):
                if score_payload.get("success") is False:
                    raise RuntimeError(score_payload.get("message") or "Remote-CaptchaAnzahlTestFehlgeschlagen")
                token_value = score_payload.get("token")
                verify_elapsed_ms = int(score_payload.get("verify_elapsed_ms") or 0)
                verify_http_status = score_payload.get("verify_http_status")
                verify_result = score_payload.get("verify_result") if isinstance(score_payload.get("verify_result"), dict) else {}
            ...
            return {
                "success": False,
                "message": f"wennvorCaptcha-MethodenichtUnterstuetztAnzahlTest: {captcha_method}",
                "captcha_method": captcha_method,
                "website_url": website_url,
                "website_key": website_key,
                "action": action,
        ...
        # RemoteProzesshatHeaderCaptcha-Loesung custom-score kannkannDurchSeiteinnerhalbDirektAbgeschlossenValidierung, 
        # inTeilImplementierunginnichtwirdAnzeigenFormatZurueckUebertragen token, Lokalnach verify_result FallbackBestimmen。
        ...
        if not token_value:
            return {
                "success": False,
                "message": "nichtAbrufenauf reCAPTCHA token",
                "captcha_method": captcha_method,
                "website_url": website_url,
                "website_key": website_key,
                "action": action,
        ...
        return {
            ...
            "message": "AnzahlValidierungErfolgreich" if verify_success else "AnzahlValidierungnichtDurch",
            "captcha_method": captcha_method,
            "website_url": website_url,
            "website_key": website_key,
            "action": action,
        ...
        return {
            "success": False,
            "message": f"AnzahlTestFehlgeschlagen: {str(e)}",
            "captcha_method": captcha_method,
            "website_url": website_url,
            "website_key": website_key,
            "action": action,
...
async def update_plugin_config(
    ...
    auto_enable_on_update = request.get("auto_enable_on_update", True)  # StandardoeffnenStarten
    ...
    return {
        "success": True,
        "message": "PluginKomponenteKonfigurationAktualisierenErfolgreich",
        "connection_token": connection_token,
        "auto_enable_on_update": auto_enable_on_update
    }
