...
class ProxyManager:
    ...
    def _parse_proxy_line(self, line: str) -> Optional[str]:
        """wirdBenutzerEingabeProxyKonvertierenfuerStandard URL Format。
        ...
        if line.startswith(("http://", "https://", "socks5://", "socks5h://")):
            # socks5h Einheitlich socks5, FuernachFortsetzenVerarbeiten
            if line.startswith("socks5h://"):
                line = "socks5://" + line[len("socks5h://"):]

            # bereitsistStandard user:pass@host:port(Oder host:port)
            if "@" in line:
                return line

            # Kompatibel protocol://host:port:user:pass
            try:
        ...
        # kein, Mit @: Standardnach http Verarbeiten
        if "@" in line:
            return f"http://{line}"

        # kein, nachDoppelpunktZeichenAnzahlMengeBestimmen
        parts = line.split(":")
        if len(parts) == 2 and parts[1].isdigit():
            # host:port
    ...
    def normalize_proxy_url(self, proxy_url: Optional[str]) -> Optional[str]:
        """StandardisierenProxy-Adresse, leerWertZurueckgeben None, Nicht-MethodeFormatWerfen ValueError。"""
        ...
        if not parsed:
            raise ValueError(
                "Proxy-AdresseFormatFehler, UnterstuetztAnzeigenBeispiel: "
                "http://user:pass@host:port / "
                "socks5://user:pass@host:port / "
    ...
    async def get_proxy_url(self) -> Optional[str]:
        """KompatibelaltAufrufenverwenden: ZurueckgebenAnfrage-ProxyAdresse"""
