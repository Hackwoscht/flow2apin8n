"""
BasisFuer RT Lokal reCAPTCHA Captcha-Dienst (Ultimativ - kein fake_useragent Rein)
Unterstuetzt: Auto-Aktualisierung Session Token, ausserhalbTeilsendenFingerabdruck-Wechsel, Retry
"""
import os
import sys
import subprocess
import signal
# Aendern Windows auf playwright  asyncio KompatibelProblem
...
# ==================== playwright Automatische Installation ====================
...
def _run_playwright_install(use_mirror: bool = False) -> bool:
    """Installieren playwright chromium Browser"""
    cmd = [sys.executable, '-m', 'playwright', 'install', 'chromium']
    env = os.environ.copy()
    
    if use_mirror:
        # VerwendenLandinnerhalbMirror
        env['PLAYWRIGHT_DOWNLOAD_HOST'] = 'https://npmmirror.com/mirrors/playwright'
    
    try:
        debug_logger.log_info("[BrowserCaptcha] GeradeinInstallieren chromium Browser...")
        print("[BrowserCaptcha] GeradeinInstallieren chromium Browser...")
        result = subprocess.run(cmd, capture_output=True, text=True, timeout=600, env=env)
        if result.returncode == 0:
            debug_logger.log_info("[BrowserCaptcha] ✅ chromium BrowserInstallierenErfolgreich")
            print("[BrowserCaptcha] ✅ chromium BrowserInstallierenErfolgreich")
            return True
        else:
            debug_logger.log_warning(f"[BrowserCaptcha] chromium InstallierenFehlgeschlagen: {result.stderr[:200]}")
            return False
    except Exception as e:
        debug_logger.log_warning(f"[BrowserCaptcha] chromium InstallierenAusnahme: {e}")
        return False


def _ensure_playwright_installed() -> bool:
    """Sicherstellen playwright bereitsInstallieren"""
    try:
        import playwright
        debug_logger.log_info("[BrowserCaptcha] playwright bereitsInstallieren")
        return True
    except ImportError:
        pass
    
    debug_logger.log_info("[BrowserCaptcha] playwright nichtInstallieren, oeffnenAnfangAutomatische Installation...")
    print("[BrowserCaptcha] playwright nichtInstallieren, oeffnenAnfangAutomatische Installation...")
    ...
    debug_logger.log_error("[BrowserCaptcha] ❌ playwright Automatische InstallationFehlgeschlagen, BitteManuellInstallieren: pip install playwright")
    print("[BrowserCaptcha] ❌ playwright Automatische InstallationFehlgeschlagen, BitteManuellInstallieren: pip install playwright")
    return False


def _ensure_browser_installed() -> bool:
    """Sicherstellen chromium BrowserbereitsInstallieren"""
    try:
        detect_script = (
        ...
        if result.returncode == 0 and browser_path and os.path.exists(browser_path):
            debug_logger.log_info(f"[BrowserCaptcha] chromium BrowserbereitsInstallieren: {browser_path}")
            return True
    except Exception as e:
        debug_logger.log_info(f"[BrowserCaptcha] BrowserbeiFehler: {e}")
    
    debug_logger.log_info("[BrowserCaptcha] chromium BrowsernichtInstallieren, oeffnenAnfangAutomatische Installation...")
    print("[BrowserCaptcha] chromium BrowsernichtInstallieren, oeffnenAnfangAutomatische Installation...")
    ...
    debug_logger.log_error("[BrowserCaptcha] ❌ chromium BrowserAutomatische InstallationFehlgeschlagen, BitteManuellInstallieren: python -m playwright install chromium")
    print("[BrowserCaptcha] ❌ chromium BrowserAutomatische InstallationFehlgeschlagen, BitteManuellInstallieren: python -m playwright install chromium")
    return False


# VersuchenversuchenImportieren playwright
...
if DOCKER_HEADED_BLOCKED:
    debug_logger.log_warning(
        "[BrowserCaptcha] auf Docker Umgebung, StandardDeaktivierenhatHeaderBrowser-Captcha-Loesung。"
        "wiebenoetigenAktivierenBitteSetzen ALLOW_DOCKER_HEADED_CAPTCHA=true, undEinreichenBereitstellen DISPLAY/Xvfb。"
    )
    print("[BrowserCaptcha] ⚠️ auf Docker Umgebung, StandardDeaktivierenhatHeaderBrowser-Captcha-Loesung")
    ...
    if IS_DOCKER and ALLOW_DOCKER_HEADED:
        debug_logger.log_warning(
            "[BrowserCaptcha] Docker hatHeaderBrowser-Captcha-LoesungNameEinzelnbereitsAktivieren, BitteSicherstellen DISPLAY/Xvfb Verfuegbar"
        )
        print("[BrowserCaptcha] ✅ Docker hatHeaderBrowser-Captcha-LoesungNameEinzelnbereitsAktivieren")
    if _ensure_playwright_installed():
        try:
            from playwright.async_api import async_playwright, Route, BrowserContext
            PLAYWRIGHT_AVAILABLE = True
            # PruefenundInstallierenBrowser
            _ensure_browser_installed()
        except ImportError as e:
            debug_logger.log_error(f"[BrowserCaptcha] playwright ImportierenFehlgeschlagen: {e}")
            print(f"[BrowserCaptcha] ❌ playwright ImportierenFehlgeschlagen: {e}")
...
def normalize_browser_proxy_url(proxy_url: str) -> tuple[Optional[str], Optional[str]]:
    """wirdBrowserProxyStandardisierenfuer Playwright/Chromium kannAkzeptierenFormat。

    Chromium nichtUnterstuetztMitKontoPasswort socks5 Proxy-Authentifizierung。
    fuerFuer `socks5://user:pass@host:port`, Automatisch herabstufenfuer `http://user:pass@host:port`, 
    KompatibelGleichbeiEinreichenBereitstellen HTTP/SOCKS5 EingangProxyDienst。
    ...
    if protocol == "socks5" and username and password:
        ...
        warning = (
            "aufMitAuthentifizierung SOCKS5 Proxy。"
            "Chromium nichtUnterstuetzt socks5 BenutzerNamePasswortAuthentifizierung, "
            f"bereitsAutomatischAendernverwenden HTTP ProxyStartenBrowser: http://{host}:{port}"
        )
...
def validate_browser_proxy_url(proxy_url: str) -> tuple[bool, str]:
    ...
    if not parsed: return False, "Proxy-FormatFehler"
    return True, None

class TokenBrowser:
    """VereinfachtBrowser: jedeMalAbrufen token beiStartenneuBrowser, verwendenAbgeschlossenSofortschliessen
    
    jedeMalalleistneuMaschine UA, VermeidenLangbeiZwischenAusfuehrenVerursachenVerschiedeneProblem
    """
    ...
    async def _create_browser(self, token_proxy_url: Optional[str] = None, manage_slot_pid: bool = True) -> tuple:
        """Create a browser instance with timeout guard and robust error handling.
        
        Shared-slot browsers track PIDs while temporary browsers do not.
        Added: 60s overall timeout to prevent hanging browser launches in Docker.
        """
        ...
        playwright = None
        try:
            # Wrap entire browser creation in a timeout to prevent indefinite hangs
            async def _do_create():
                ...
                browser_args = [
                    ...
                    '--disable-infobars',
                    '--hide-scrollbars',
                    # Extra stability flags for Docker/Xvfb
                    '--disable-gpu',
                ...
                debug_logger.log_info(
                    f"[BrowserCaptcha] Token-{self.token_id} shared browser started (proxy={'yes' if raw_proxy_url else 'no'}, pid={browser_pid})"
                )
                return playwright, browser, context

            # 60s timeout for the entire browser creation process
            return await asyncio.wait_for(_do_create(), timeout=60)

        except asyncio.TimeoutError:
            debug_logger.log_error(
                f"[BrowserCaptcha] Token-{self.token_id} browser launch timed out after 60s"
            )
            try:
                if playwright:
            ...
            if manage_slot_pid:
                self._write_pid_file(None)
            raise Exception("Browser launch timed out after 60s")
        except Exception as e:
    ...
    async def _get_or_create_shared_browser(self, token_proxy_url: Optional[str] = None) -> tuple:
        """Get or create the shared browser for this slot with robust health checks."""
        ...
        async with self._shared_browser_lock:
            ...
            if has_shared_browser:
                ...
                if not is_connected:
                    debug_logger.log_warning(
                        f"[BrowserCaptcha] Token-{self.token_id} shared browser disconnected, recycling"
                    )
            ...
            # Health check: verify context is still usable
            if has_shared_browser:
                try:
                    await self._ensure_shared_keepalive_page()
                except Exception as e:
                    debug_logger.log_warning(
                        f"[BrowserCaptcha] Token-{self.token_id} keepalive page broken: {type(e).__name__}, recycling"
                    )
                    await self._recycle_browser_locked(reason="keepalive_page_broken", rotate_profile=False)
                    has_shared_browser = False

            # Reuse limit: recycle after too many reuses to prevent stale state accumulation
            max_reuse = 50
            if has_shared_browser and self._shared_reuse_count >= max_reuse:
                debug_logger.log_info(
                    f"[BrowserCaptcha] Token-{self.token_id} shared browser reached {max_reuse} reuses, proactive recycle"
                )
                await self._recycle_browser_locked(reason="max_reuse_reached", rotate_profile=True)
                has_shared_browser = False
    ...
    async def _capture_page_fingerprint(self, page):
        """vonBrowserSeiteExtrahieren UA mitClient-Hint-Header, SicherstellenmitCaptcha-BrowserKonsistent。"""
        try:
            ...
            debug_logger.log_warning(f"[BrowserCaptcha] Token-{self.token_id} ExtrahierenBrowser-FingerabdruckFehlgeschlagen: {type(e).__name__}: {str(e)[:200]}")
    ...
    async def _wait_and_close_after_request(
        ...
        """WartenUpstreamAnfrageBeendetnachErneutSchliessenBrowser(TimeoutFallback)。"""
        close_reason = "UpstreamAnfrageAbgeschlossen"
        try:
            await asyncio.wait_for(release_event.wait(), timeout=wait_timeout)
        except asyncio.TimeoutError:
            close_reason = f"WartenUpstreamAnfrageAbgeschlossenTimeout({wait_timeout}s)"
            debug_logger.log_warning(
                f"[BrowserCaptcha] Token-{self.token_id} {close_reason}, AusfuehrenFallbackSchliessen"
            )
        except Exception as e:
            close_reason = f"WartenUpstreamAnfrageAbgeschlossenAusnahme: {type(e).__name__}"
            debug_logger.log_warning(
                f"[BrowserCaptcha] Token-{self.token_id} {close_reason}, AusfuehrenFallbackSchliessen"
            )
        finally:
            ...
            debug_logger.log_info(
                f"[BrowserCaptcha] Token-{self.token_id} {close_reason}, BrowserbereitsSchliessen (action={action}, request_ref={request_ref[:8]})"
            )
    ...
    async def _defer_browser_close_until_request_done(
        ...
        """Captcha-LoesungErfolgreichnachVerzoegerungSchliessenBrowser, Warten Flow AnfrageBeendetBenachrichtigung。"""
        ...
        if action == "IMAGE_GENERATION":
            # BildRoutekannkannEnthaeltUpscaleAnfrage, WartenaufwenigeUeberschreiben flow/upsample Timeout
            base_timeout = max(flow_timeout, upsample_timeout)
            wait_timeout = max(base_timeout + 180, 900)
        else:
            # VideoAnfrageStandardTimeoutLang, gebengrossPufferVermeiden"AnfragenichtBeendetSchliessen"
        ...
        debug_logger.log_info(
            f"[BrowserCaptcha] Token-{self.token_id} Captcha-LoesungErfolgreichnachEintretenVerzoegerungSchliessen, WartenUpstreamAnfrageAbgeschlossen "
    ...
    async def notify_generation_request_finished(self, request_ref: Optional[str] = None):
        """Benachrichtigungwennvor Token fuerAntwortUpstreamBild/VideoAnfragebereitsBeendet。"""
        async with self._pending_release_lock:
            release_event = None
            ...
            if matched_ref and matched_ref in self._pending_release_entries:
                ...
                # KompatibelaltAufrufenverwenden(kein request_ref), NurZurueckempfangenAeltestewartenFreigebenElement, VermeideneinMalBeeinflussenalleTeilAnfrage。
        ...
        if release_event and not release_event.is_set():
            release_event.set()
            debug_logger.log_info(
                f"[BrowserCaptcha] Token-{self.token_id} empfangenaufUpstreamAnfrageAbgeschlossenBenachrichtigung, oeffnenAnfangSchliessenBrowser "
    ...
    async def _execute_captcha(self, context, project_id: str, website_key: str, action: str) -> Optional[str]:
        """Execute captcha solving logic in the given browser context.
        
        Improvements over original:
        - reload/clr events are monitored BEFORE execute() so we never miss early responses
        - If token was obtained but reload/clr timeout, we still return the token (soft fail)
        - Increased timeouts for Docker/Xvfb environments
        - Better error logging with timing info
        """
        page = None
        ...
        try:
            ...
            def handle_request_failed(request):
                try:
                    ...
                    debug_logger.log_warning(
                        f"[BrowserCaptcha] Token-{self.token_id} resource load failed: url={failed_url[:200]}, error={failure}"
                    )
                except Exception:
                    pass
            ...
            # Set up reload/clr event listeners BEFORE navigation so we never miss early responses
            ...
            # Navigate with increased timeout for slow Docker/Xvfb environments
            try:
                await page.goto(page_url, wait_until="load", timeout=45000)
            except Exception as e:
                elapsed = time.time() - captcha_start
                debug_logger.log_warning(
                    f"[BrowserCaptcha] Token-{self.token_id} page.goto failed after {elapsed:.1f}s: {type(e).__name__}: {str(e)[:200]}"
                )
                return None
            
            # Wait for grecaptcha with increased timeout
            try:
                await page.wait_for_function("typeof grecaptcha !== 'undefined'", timeout=20000)
            except Exception as e:
                elapsed = time.time() - captcha_start
                debug_logger.log_warning(
                    f"[BrowserCaptcha] Token-{self.token_id} grecaptcha not ready after {elapsed:.1f}s: {type(e).__name__}: {str(e)[:200]}"
                )
                return None

            # Also wait for grecaptcha.enterprise.execute to be available
            try:
                ...
                debug_logger.log_warning(
                    f"[BrowserCaptcha] Token-{self.token_id} grecaptcha.enterprise.execute not available: {type(e).__name__}: {str(e)[:200]}"
                )
                return None

            # Capture page fingerprint for downstream request consistency
            await self._capture_page_fingerprint(page)
            
            # Execute reCAPTCHA with increased timeout
            token = None
            try:
                ...
                elapsed = time.time() - captcha_start
                debug_logger.log_warning(
                    f"[BrowserCaptcha] Token-{self.token_id} grecaptcha.execute failed after {elapsed:.1f}s: {type(e).__name__}: {str(e)[:200]}"
                )
                return None

            if not token:
                debug_logger.log_warning(f"[BrowserCaptcha] Token-{self.token_id} grecaptcha.execute returned empty token")
                return None

            # Wait for enterprise/reload and enterprise/clr responses.
            # These confirm the token is fully validated server-side.
            # SOFT FAIL: if token was obtained but reload/clr timeout, still return the token
            # since it may still be valid — Google sometimes skips clr on fast solves.
            reload_ok = True
            clr_ok = True
            ...
            try:
                ...
                reload_ok = False
                debug_logger.log_warning(
                    f"[BrowserCaptcha] Token-{self.token_id} enterprise/reload 200 timeout (soft fail, token still returned)"
                )
            ...
            try:
                ...
                clr_ok = False
                debug_logger.log_warning(
                    f"[BrowserCaptcha] Token-{self.token_id} enterprise/clr 200 timeout (soft fail, token still returned)"
                )

            # Post-settle wait for enterprise chain stability
            post_wait_seconds = float(getattr(config, "browser_recaptcha_settle_seconds", 3) or 3)
            # If reload/clr both succeeded, use configured wait; if one failed, wait a bit longer
            if not reload_ok or not clr_ok:
                post_wait_seconds = max(post_wait_seconds, 4.0)

            if post_wait_seconds > 0:
                debug_logger.log_info(
                    f"[BrowserCaptcha] Token-{self.token_id} reload={'ok' if reload_ok else 'timeout'} "
                    f"clr={'ok' if clr_ok else 'timeout'}, settling {post_wait_seconds:.1f}s before returning token"
                )
                await asyncio.sleep(post_wait_seconds)

            elapsed = time.time() - captcha_start
            debug_logger.log_info(
                f"[BrowserCaptcha] Token-{self.token_id} captcha solved in {elapsed:.1f}s "
                f"(reload={'ok' if reload_ok else 'miss'}, clr={'ok' if clr_ok else 'miss'})"
            )
            return token
            ...
            debug_logger.log_warning(f"[BrowserCaptcha] Token-{self.token_id} captcha failed after {elapsed:.1f}s: {msg[:200]}")
            return None
        finally:
            if page:
                try:
    ...
    async def _execute_custom_captcha(
        ...
        """inBeliebigSeiteAusfuehren reCAPTCHA, verwendenFuerAnzahlTestNicht- Flow Szenario。"""
        page = None
        try:
            page = await context.new_page()
            await page.add_init_script("Object.defineProperty(navigator, 'webdriver', {get: () => undefined});")
            ...
            debug_logger.log_info(
                f"[BrowserCaptcha] Token-{self.token_id} LadenwahrTatsaechlichDefinitionSeite {api_label}: primary={primary_host}, secondary={secondary_host}, url={website_url}"
            )
            ...
            def handle_request_failed(request):
                try:
                    ...
                    failure = request.failure or ""
                    debug_logger.log_warning(
                        f"[BrowserCaptcha] Token-{self.token_id} DefinitionRessourceLadenFehlgeschlagen: url={failed_url[:200]}, error={failure}"
                    )
                except Exception:
                    pass
            ...
            try:
                ...
                debug_logger.log_warning(
                    f"[BrowserCaptcha] Token-{self.token_id} Definition page.goto Fehlgeschlagen: {type(e).__name__}: {str(e)[:200]}"
                )
                return None
            ...
            for _ in range(20):
                ...
                await asyncio.sleep(0.5)
            if not page_loaded:
                debug_logger.log_warning(f"[BrowserCaptcha] Token-{self.token_id} DefinitionSeite readyState nichtErreichenauf complete, FortsetzenVersuchenversuchenAufwaermen")

            # SimulierenNatuerlichvorKonsoleInteraktion, VermeidenKaltStartenleeraufunterTextDirekt execute。
            try:
            ...
            if warmup_seconds > 0:
                debug_logger.log_info(
                    f"[BrowserCaptcha] Token-{self.token_id} wahrTatsaechlichSeiteAufwaermen {warmup_seconds:.1f}s nachErneutAusfuehrenDefinitionCaptcha-Loesung"
                )
            ...
            try:
                ...
                debug_logger.log_warning(
                    f"[BrowserCaptcha] Token-{self.token_id} Definition grecaptcha nichtBereit, VersuchenversuchenErgaenzenInjizierenSkript: {type(e).__name__}: {str(e)[:200]}"
                )
                try:
                    ...
                    debug_logger.log_warning(
                        f"[BrowserCaptcha] Token-{self.token_id} Definition grecaptcha Am meistenEndgueltignichtBereit: {type(inject_error).__name__}: {str(inject_error)[:200]}"
                    )
                    return None
            ...
            if post_wait_seconds > 0:
                debug_logger.log_info(
                    f"[BrowserCaptcha] Token-{self.token_id} DefinitionCaptcha-LoesungbereitsAbgeschlossen, ZusaetzlichausserhalbWarten {post_wait_seconds:.1f}s nachZurueckgeben token"
                )
                await asyncio.sleep(post_wait_seconds)
            ...
            msg = f"{type(e).__name__}: {str(e)}"
            debug_logger.log_warning(f"[BrowserCaptcha] Token-{self.token_id} DefinitionCaptcha-LoesungFehlgeschlagen: {msg[:200]}")
            return None
        finally:
            if page:
                try:
    ...
    def get_last_fingerprint(self) -> Optional[Dict[str, Any]]:
        """ZurueckgebenAm meistenLetzteeinMalCaptcha-BrowserFingerabdruck-Snapshot。"""
    ...
    async def get_token(
        ...
        """Get a reCAPTCHA token from the shared browser with bulletproof error recovery.
        
        Improvements:
        - Exponential backoff between retries instead of fixed 1s
        - Browser health check before each attempt
        - Proactive browser recycle after any failure (not just consecutive)
        - Catches playwright-specific errors that indicate stale browser state
        - Returns partial error info for upstream diagnostics
        """
            ...
            try:
                ...
                for attempt in range(max_retries):
                    ...
                    try:
                        ...
                        # Pre-flight: verify shared browser is still alive before attempting captcha
                        if self._shared_browser:
                            try:
                                checker = getattr(self._shared_browser, "is_connected", None)
                                if callable(checker) and not checker():
                                    debug_logger.log_warning(
                                        f"[BrowserCaptcha] Token-{self.token_id} pre-flight: browser disconnected, recycling"
                                    )
                                    await self.recycle_browser(reason="preflight_disconnected", rotate_profile=False)
                            except Exception:
                                await self.recycle_browser(reason="preflight_check_error", rotate_profile=False)
                        ...
                        if token:
                            ...
                            debug_logger.log_info(
                                f"[BrowserCaptcha] Token-{self.token_id} token acquired in {elapsed_ms:.0f}ms "
                                f"(attempt={attempt+1}, launches={self._shared_launch_count}, reuse={self._shared_reuse_count})"
                            )
                            return token, None

                        # Token was None — captcha execution failed but no exception
                        self._error_count += 1
                        self._consecutive_browser_failures += 1
                        last_error_msg = "captcha returned None"
                        debug_logger.log_warning(
                            f"[BrowserCaptcha] Token-{self.token_id} attempt {attempt+1}/{max_retries} failed: captcha returned None"
                        )

                        # Always recycle after failure to get a fresh browser state
                        # On first failure: keep profile. On subsequent: rotate profile for fresh fingerprint.
                        rotate = self._consecutive_browser_failures >= 2
                        await self.recycle_browser(
                            reason=f"captcha_null_attempt_{attempt+1}",
                            rotate_profile=rotate
                        )
                        ...
                        debug_logger.log_error(
                            f"[BrowserCaptcha] Token-{self.token_id} attempt {attempt+1}/{max_retries} exception: {last_error_msg}"
                        )

                        # Detect fatal browser errors that require immediate recycle
                        error_lower = error_message.lower()
                        is_browser_dead = any(keyword in error_lower for keyword in [
                        ...
                        if is_browser_dead:
                            debug_logger.log_warning(
                                f"[BrowserCaptcha] Token-{self.token_id} fatal browser error detected, forcing full recycle"
                            )
                            await self.recycle_browser(reason="fatal_browser_error", rotate_profile=True)
                        else:
                            # Non-fatal error: recycle without profile rotation
                            await self.recycle_browser(reason="captcha_exception", rotate_profile=False)

                    # Exponential backoff: 1s, 2s, 4s between retries
                    if attempt < max_retries - 1:
                        backoff = min(2 ** attempt, 4)
                        debug_logger.log_info(
                            f"[BrowserCaptcha] Token-{self.token_id} waiting {backoff}s before retry {attempt+2}/{max_retries}"
                        )
                        await asyncio.sleep(backoff)

                debug_logger.log_error(
                    f"[BrowserCaptcha] Token-{self.token_id} all {max_retries} attempts exhausted. Last error: {last_error_msg}"
                )
                return None, None
            finally:
...
class BrowserCaptchaService:
    """vieleBrowserRotationCaptcha-Dienst(Singleton-Modus)
    
    UnterstuetztKonfigurationBrowserAnzahlMenge, jedeStueckBrowserNuroeffnen 1 StueckTab, AnfrageRotationszuweisung
    """
        ...
        if cls._instance is None:
            async with cls._lock:
                if cls._instance is None:
                    ...
                    # vonDatenbankLaden browser_count Konfiguration
    ...
    def _check_available(self):
        ...
        if DOCKER_HEADED_BLOCKED:
            raise RuntimeError(
                "auf Docker Umgebung, StandardDeaktivierenhatHeaderBrowser-Captcha-Loesung。"
        ...
        if IS_DOCKER and not os.environ.get("DISPLAY"):
            raise RuntimeError(
                "Docker hatHeaderBrowser-Captcha-LoesungbereitsAktivieren, aber DISPLAY nichtSetzen。"
                "BitteSetzen DISPLAY(Beispielwie :99)undStarten Xvfb。"
            )
        if not PLAYWRIGHT_AVAILABLE or async_playwright is None:
            raise RuntimeError(
                "playwright nichtInstallierenOdernichtVerfuegbar。"
                "BitteManuellInstallieren: pip install playwright && python -m playwright install chromium"
            )
    
    async def _load_browser_count(self):
        """vonDatenbankLadenBrowserAnzahlMengeKonfiguration"""
        if self.db:
            try:
                captcha_config = await self.db.get_captcha_config()
                self._browser_count = max(1, captcha_config.browser_count)
                debug_logger.log_info(f"[BrowserCaptcha] BrowserAnzahlMengeKonfiguration: {self._browser_count}")
            except Exception as e:
                debug_logger.log_warning(f"[BrowserCaptcha] Laden browser_count KonfigurationFehlgeschlagen: {e}, VerwendenStandardWert 1")
                self._browser_count = 1
        # Parallelitaetsbegrenzung = BrowserAnzahlMenge, nichtErneutHartKodierenCodeBegrenzung
        self._token_semaphore = asyncio.Semaphore(self._browser_count)
        debug_logger.log_info(f"[BrowserCaptcha] Parallelitaets-Obergrenze: {self._browser_count}")
    ...
    async def _select_browser_id(self, project_id: Optional[str]) -> int:
        ...
        # AuswahlUndReservierenMussAtomarisieren, ObdannGleichBatchParallelitaetwirdfuerUndWiederholtTreffeninGleich slot。
        async with self._slot_allocation_lock:
    ...
    async def _get_or_create_browser(self, browser_id: int) -> TokenBrowser:
        """AbrufenOderErstellenAngegeben ID BrowserInstanz"""
        async with self._browsers_lock:
            if browser_id not in self._browsers:
                ...
                debug_logger.log_info(f"[BrowserCaptcha] ErstellenBrowserInstanz {browser_id}")
            return self._browsers[browser_id]
    
    def _get_next_browser_id(self) -> int:
        """Per Rotation abrufenuntereinStueckBrowser ID"""
    ...
    def _compose_browser_ref(browser_id: int, request_ref: Optional[str]) -> Union[int, str]:
        """wird browser_id mit request_ref ZusammenfuehrenfuerkannZurueckUebertragenAnfrageHandle。"""
        if request_ref:
    ...
    def _parse_browser_ref(browser_ref: Optional[Union[int, str]]) -> tuple[Optional[int], Optional[str]]:
        """ParsenAnfrageHandle, KompatibelaltRein int browser_id。"""
    ...
    async def _resolve_token_proxy_url(self, token_id: Optional[int]) -> Optional[str]:
        """lesenabrufen token LevelCaptcha-Proxy, fuerleerbeiFallbackalleKonfiguration。"""
        ...
        try:
            ...
            debug_logger.log_warning(f"[BrowserCaptcha] lesenabrufen token({token_id}) Captcha-ProxyFehlgeschlagen: {e}")
        return None
    
    async def get_token(self, project_id: str, action: str = "IMAGE_GENERATION", token_id: int = None) -> tuple[Optional[str], Union[int, str]]:
        """Abrufen reCAPTCHA Token(RotationszuweisungaufnichtGleichBrowser)
        
        Args:
            project_id: Projekt ID
            action: reCAPTCHA action
            token_id: Geschaeft token id(NurverwendenFuerlesenabrufen token LevelCaptcha-Proxy)
        
        Returns:
            (token, browser_ref) Gruppe, browser_ref Enthaelt browser_id mitAnfrageLevel request_ref
        """
        # PruefenDienstistObVerfuegbar
        ...
        # alleParallelitaetsbegrenzung(wieErgebnisbereitsKonfiguration)
        if self._token_semaphore:
            async with self._token_semaphore:
                # Per Rotation waehlenBrowser
        ...
        # keinParallelitaetsbegrenzungbeiDirektAusfuehren
        browser_id = await self._select_browser_id(project_id)
        try:
            browser = await self._get_or_create_browser(browser_id)
            token, request_ref = await browser.get_token(
    ...
    async def get_custom_token(
        ...
        """AbrufenBeliebigSeite reCAPTCHA token, verwendenFuerAnzahlTest。"""
        self._check_available()
    ...
    async def get_custom_score(
        ...
        """inBrowserSeiteinnerhalbAbgeschlossen token AbrufenmitAnzahlValidierung。"""
        self._check_available()
    ...
    async def get_fingerprint(self, browser_ref: Optional[Union[int, str]]) -> Optional[Dict[str, Any]]:
        """AbrufenAngegebenBrowserAm meistenLetzteeinMalCaptcha-LoesungbeiFingerabdruck-Snapshot。"""
    ...
    async def report_error(self, browser_ref: Optional[Union[int, str]] = None, error_reason: Optional[str] = None):
        """Handle upstream errors; recycle browser on reCAPTCHA failures AND HTTP 403/500 errors.
        
        Bulletproof: any 403 likely means the captcha token was rejected by Google,
        so we always recycle with profile rotation to get a fresh fingerprint.
        """
        browser_id, _ = self._parse_browser_ref(browser_ref)
        ...
        async with self._browsers_lock:
            ...
            # Recycle on any of these conditions:
            should_recycle = False
            recycle_reason = error_reason or "unknown"

            # 1. Explicit reCAPTCHA failures
            if "recaptcha" in error_lower and any(kw in error_lower for kw in [
                "evaluation failed", "verification failed", "failed", "invalid", "expired"
            ]):
                should_recycle = True
                recycle_reason = "recaptcha_rejected"
                self._stats["api_403"] += 1

            # 2. HTTP 403 — almost always means captcha token was invalid
            elif "403" in error_lower or "forbidden" in error_lower:
                should_recycle = True
                recycle_reason = "http_403_captcha_likely_invalid"
                self._stats["api_403"] += 1

            # 3. HTTP 500 from upstream that mentions captcha/token
            elif ("500" in error_lower or "internal server error" in error_lower) and any(
                kw in error_lower for kw in ["captcha", "token", "recaptcha", "auth"]
            ):
                should_recycle = True
                recycle_reason = "http_500_captcha_related"

            if browser_id is not None:
                debug_logger.log_info(
                    f"[BrowserCaptcha] browser {browser_id} error reported: reason={error_reason or 'unknown'}, recycle={should_recycle}"
                )
        ...
        if browser and should_recycle:
            try:
                await browser.recycle_browser(
                    ...
                    rotate_profile=True,  # Always rotate on error for fresh fingerprint
                )
            except Exception as e:
    ...
    async def report_request_finished(self, browser_ref: Optional[Union[int, str]] = None):
        """aufEbeneBenachrichtigungMalAnfragebereitsAbgeschlossen; browser ModusNurBeibehaltenResidentBrowser, nichtinErfolgreichnachAktivSchliessen。"""
        browser_id, _ = self._parse_browser_ref(browser_ref)
        if browser_id is None:
            return
