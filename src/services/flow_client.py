...
class FlowClient:
    ...
    def __init__(self, proxy_manager, db=None):
        ...
        # CachejedeStueckKonto User-Agent
        self._user_agent_cache = {}
        # wennvorAnfrageRouteGebundenBrowser-Fingerabdruck(BasisFuer contextvar, VermeidenParallelitaetInterferenz)
        ...
        # sendenDispatchStrategieAendernfuer"Anfrageaufsenden": 
        # nichtin flow2api LokalfuerEinreichenUebergebenmachenBatchMalShapingOderWarteschlange, VermeidenwirdGleichBatchAnfrageErfolgreichStaffelung。

    def _generate_user_agent(self, account_id: str = None) -> str:
        """BasisFuerKontoIDGenerierungFestBestimmen User-Agent
        
        Args:
            account_id: KontoKennung(wie email Oder token_id), GleichKontoZurueckgebenGleich UA
            
        Returns:
            User-Agent String
        """
        # wieErgebnisKeinhatEinreichenBereitstellenKontoID, GenerierungMaschineUA
        if not account_id:
            account_id = f"random_{random.randint(1, 999999)}"
        
        # wieErgebnisbereitsCache, DirektZurueckgeben
        if account_id in self._user_agent_cache:
            return self._user_agent_cache[account_id]
        
        # VerwendenKontoIDfuerMaschineSeed, SicherstellenGleichKontoGenerierungGleichUA
        import hashlib
        ...
        # VerwendenFestBestimmenSeedMaschineAuswahlBetriebssystemUndBrowser
    ...
    def _set_request_fingerprint(self, fingerprint: Optional[Dict[str, Any]]):
        """SetzenwennvorAnfrageRouteBrowserFingerabdruck-Kontext。"""
    ...
    def get_request_fingerprint(self) -> Optional[Dict[str, Any]]:
        """AbrufenwennvorAnfrageRouteGebundenBrowser-FingerabdruckSnapshot。"""
    ...
    def clear_request_fingerprint(self):
        """BereinigenAnfrageRouteGebundenBrowser-Fingerabdruck。"""
    ...
    async def _make_request(
        ...
        """EinheitlichHTTPAnfrageVerarbeiten

        Args:
            method: HTTPMethode (GET/POST)
            url: AbgeschlossenGesamtURL
            headers: AnfrageHeader
            json_data: JSONAnfrageBody
            use_st: istObVerwendenSTAuthentifizierung (CookieFormat)
            st_token: Session Token
            use_at: istObVerwendenATAuthentifizierung (BearerFormat)
            at_token: Access Token
            timeout: DefinitionTimeoutbeiZwischen(Sekunden), nichtUebertragendannVerwendenStandardWert
            use_media_proxy: istObVerwendenBild-Upload/Download-Proxy
            respect_fingerprint_proxy: istObBevorzugtVerwendenCaptcha-BrowserZeigenFingerabdruckinProxy
        """
        fingerprint = self._request_fingerprint_ctx.get()
        ...
        # STAuthentifizierung - VerwendenCookie
        if use_st and st_token:
            headers["Cookie"] = f"__Secure-next-auth.session-token={st_token}"

        # ATAuthentifizierung - VerwendenBearer
        if use_at and at_token:
            headers["authorization"] = f"Bearer {at_token}"

        # SicherstellenBestimmenKontoKennung(BevorzugtVerwenden token vor16StueckEntsprechenfuerKennung)
        account_id = None
        if st_token:
            account_id = st_token[:16]  # Verwenden ST vor16StueckEntsprechen
        elif at_token:
            account_id = at_token[:16]  # Verwenden AT vor16StueckEntsprechen

        # DurchverwendenAnfrageHeader - BevorzugtVerwendenCaptcha-BrowserZeigenFingerabdruckin UA
        ...
        # fallsspeicherninCaptcha-BrowserZeigenFingerabdruck, UeberschreibenschliessenSchluesselClient-Hint-Header, EinreichenUebergebenAnfragemitCaptcha-LoesungbeiKonsistent。
        if isinstance(fingerprint, dict):
        ...
        if config.debug_enabled:
            if isinstance(fingerprint, dict):
                ...
                debug_logger.log_info(
                    f"[FINGERPRINT] VerwendenCaptcha-BrowserZeigenFingerabdruckEinreichenUebergebenAnfrage: UA={headers.get('User-Agent', '')[:120]}, proxy={proxy_for_log}"
                )
        ...
        try:
            async with AsyncSession() as session:
                ...
                if response.status_code >= 400:
                    ...
                    try:
                        ...
                        # Extrahieren Google API FehlerFormatin reason
                        if "error" in error_body:
                            error_info = error_body["error"]
                            error_message = error_info.get("message", "")
                            # von details inExtrahieren reason
                    ...
                    # FehlgeschlagenbeiAusgabeAnfrageBodyUndFehlerinnerhalbaufKonsole
            ...
            # wieErgebnisnichtistWerfenAusnahme, Protokoll
            ...
            if self._should_fallback_to_urllib(error_msg):
                debug_logger.log_warning(
                    f"[HTTP FALLBACK] curl_cffi AnfrageFehlgeschlagen, Fallback urllib: {method.upper()} {url}"
                )
                try:
                    ...
                    debug_logger.log_error(
                        f"[HTTP FALLBACK] urllib FallbackauchFehlgeschlagen: {fallback_error}"
                    )
                    raise Exception(
    ...
    def _should_fallback_to_urllib(self, error_message: str) -> bool:
        """BestimmenistObAntwortvon curl_cffi Fallbackauf urllib。"""
    ...
    def _sync_json_request_via_urllib(
        ...
        """Verwenden urllib Ausfuehren JSON Anfrage, fuer curl_cffi NetzwerkFallback。"""
    ...
    def _is_timeout_error(self, error: Exception) -> bool:
        """BestimmenistObfuerNetzwerk-Timeout, FuerSchnellFehlgeschlagenRetry。"""
    ...
    def _is_retryable_network_error(self, error_str: str) -> bool:
        """UnterscheidenkannRetry TLS/VerbindungNetzwerkFehler。"""
    ...
    def _get_control_plane_timeout(self) -> int:
        """MengeAnfrageTimeout, VermeidenAuthentifizierung/ProjektSchnittstelleLangbeiZwischen。"""
    ...
    async def _acquire_image_launch_gate(
        ...
        """BildAnfragenichtErneutmachenLokalsendenDispatchWarteschlange, DirektEintretenabrufen token undEinreichenUebergebenUpstream。"""
        return True, 0, 0

    async def _release_image_launch_gate(self, token_id: Optional[int]):
        """BeibehaltenSchnittstelleForm, wennvorkeinbenoetigenFreigebenWelcheLokalsendenDispatchStatus。"""
        return
    ...
    async def _acquire_video_launch_gate(
        ...
        """VideoAnfragenichtErneutmachenLokalsendenDispatchWarteschlange, DirektEintretenabrufen token undEinreichenUebergebenUpstream。"""
        return True, 0, 0
    ...
    async def _make_image_generation_request(
        ...
        """Bild-GenerierungAnfrageVerwendenTimeout, undinNetzwerk-TimeoutbeiSchnellRetry。"""
        ...
        # fuerFuerBrowser/RemoteProzessBrowser-Captcha-LoesungRoute, BevorzugtmitCaptcha-LoesungbeiKonsistentAusgang。
        # ObdanninErsteSpringenAenderngehenMedien-Proxybei, senden reCAPTCHA ValidierungFehlgeschlagenundUpscaleLangEnde。
        fingerprint = self._request_fingerprint_ctx.get()
        ...
        if has_fingerprint_context and prefer_media_first:
            ...
            debug_logger.log_info(
                "[IMAGE] aufCaptcha-BrowserFingerabdruck-Kontext, ErsteSpringenFestBestimmengehenCaptcha-Route; "
                "Medien-ProxyNurinNetzwerk-TimeoutbeifuerFallbackFallback。"
            )
        ...
        for attempt_index in range(total_attempts):
            if has_media_proxy:
                # MalRetrybeiverwenden"Route + Route"Strategie, VermeidenjedeMalalleZuerstinFehlerRouteauf。
                if attempt_index == 0:
            ...
            route_label = "Medien-ProxyRoute" if prefer_media_proxy else "Captcha-Route"
            ...
            try:
                ...
                next_route_label = "Medien-ProxyRoute" if next_prefer_media_proxy else "Captcha-Route"
                debug_logger.log_warning(
                    f"[IMAGE] Bild-GenerierungAnfrageNetzwerk-Timeout, VorbereitenSchnellRetry "
                    f"({attempt_index + 2}/{total_attempts}), wennvorRoute={route_label}, "
                    f"untereinRoute={next_route_label}, timeout={request_timeout}s"
                )
                if retry_delay > 0:
        ...
        if last_error is not None:
            raise last_error
        raise RuntimeError("Bild-GenerierungAnfrageFehlgeschlagen")

    # ========== Authentifizierungsbezogen (VerwendenST) ==========
    ...
    # ========== Projektverwaltung (VerwendenST) ==========

    async def create_project(self, st: str, title: str) -> str:
        """Projekt erstellen,Zurueckgebenproject_id
        ...
        for retry_attempt in range(max_retries):
            try:
                ...
                last_error = e
                retry_reason = "Netzwerk-Timeout" if self._is_timeout_error(e) else self._get_retry_reason(str(e))
                if retry_reason and retry_attempt < max_retries - 1:
                    debug_logger.log_warning(
                        f"[PROJECT] Projekt erstellenFehlgeschlagen, VorbereitenRetry ({retry_attempt + 2}/{max_retries}) "
        ...
        if last_error is not None:
            raise last_error
        raise RuntimeError("Projekt erstellenFehlgeschlagen")
    ...
    # ========== Bild-Upload (VerwendenAT) ==========

    def _detect_image_mime_type(self, image_bytes: bytes) -> str:
        """DurchDateiHeader magic bytes Bild MIME Typ

        Args:
            image_bytes: BildAnzahlBasierend auf

        Returns:
            MIME TypString, Standard image/jpeg
        """
    ...
    def _convert_to_jpeg(self, image_bytes: bytes) -> bytes:
        """wirdBildKonvertierenfuer JPEG Format

        Args:
            image_bytes: OriginalAnfangBildAnzahlBasierend auf

        Returns:
            JPEG FormatBildAnzahlBasierend auf
        """
        from io import BytesIO
        from PIL import Image

        img = Image.open(BytesIO(image_bytes))
        # wieErgebnishatDurchleitenKlarDurch, Konvertierenfuer RGB
    ...
    async def upload_image(
        ...
        """Bild hochladen,ZurueckgebenmediaId

        Args:
            at: Access Token
            image_bytes: BildAnzahlBasierend auf
            aspect_ratio: BildOderVideoSeitenverhaeltnis(wirdAutomatischKonvertierenfuerBildFormat)
            project_id: ProjektID(neuUploadSchnittstellekannVerwenden)

        Returns:
            mediaId
        """
        # KonvertierenVideoaspect_ratiofuerBildaspect_ratio
        ...
        # AutomatischBild MIME Typ
        mime_type = self._detect_image_mime_type(image_bytes)

        # KodierenCodefuerbase64 (EntfernenEntfernenvorSuffix)
        image_base64 = base64.b64encode(image_bytes).decode('utf-8')

        # BevorzugtVersuchenversuchenneuUploadSchnittstelle: /v1/flow/uploadImage
        # fallsFehlgeschlagendannAutomatischFallbackaufaltSchnittstelle,Kompatibel
        ...
        # KompatibelFallback: altSchnittstelle :uploadUserImage
        ...
        for retry_attempt in range(max_retries):
            ...
            try:
                ...
                if retry_reason and retry_attempt < max_retries - 1:
                    debug_logger.log_warning(
                        f"[UPLOAD] UploadTreffen aufauf{retry_reason}, VorbereitenRetry ({retry_attempt + 2}/{max_retries})..."
                    )
                    await asyncio.sleep(1)
                    continue
                raise
        ...
        if last_error is not None:
            raise last_error
        raise RuntimeError("Bild hochladenFehlgeschlagen")

    # ========== Bild-Generierung (VerwendenAT) - GleichZurueckgeben ==========
    ...
    async def generate_image(
        ...
        """Bild generieren(GleichZurueckgeben)
        ...
        Args:
            ...
            aspect_ratio: BildSeitenverhaeltnis
            image_inputs: BildListe(Bild-zu-BildbeiVerwenden)

        Returns:
            (result, session_id, perf_trace)
            result: UpstreamZurueckgebenGenerierungErgebnisErgebnis
            session_id: MalErfolgreichBild-GenerierungAnfrageVerwenden sessionId
            perf_trace: GenerierungRetrymitRouteVerbrauchenbei
        """
        url = f"{self.api_base_url}/projects/{project_id}/flowMedia:batchGenerateImages"

        # 403/reCAPTCHA RetryLogik
        ...
        for retry_attempt in range(max_retries):
            ...
            # jedeMalRetryalleErneutneuAbrufen reCAPTCHA token
            ...
            # StrukturErstellenAnfrage - neuSchnittstelleinausserhalbEbeneUnd requests innerhalballeMit clientContext
            client_context = {
                "recaptchaContext": {
            ...
            # neuBildSchnittstelleVerwendenStrukturisierenPrompt + new media Schalter
            request_data = {
        ...
        # allehatRetryalleFehlgeschlagen
    ...
    async def upsample_image(
        ...
        """Bild hochskalierenauf 2K/4K

        Args:
            at: Access Token
            project_id: ProjektID
            media_id: Bild mediaId (von batchGenerateImages Zurueckgeben media[0]["name"])
            target_resolution: UPSAMPLE_IMAGE_RESOLUTION_2K Oder UPSAMPLE_IMAGE_RESOLUTION_4K
            user_paygate_tier: BenutzerLevel (wie PAYGATE_TIER_NOT_PAID / PAYGATE_TIER_ONE)
            session_id: Optional, WiederverwendenBild-GenerierungAnfrage sessionId

        Returns:
            base64 KodierenCodeBildAnzahlBasierend auf
        """
        url = f"{self.api_base_url}/flow/upsampleImage"

        # 403/reCAPTCHA/500 RetryLogik - Am meistenvieleRetry3Mal
        max_retries = 3
        last_error = None

        for retry_attempt in range(max_retries):
            # Abrufen reCAPTCHA token - Verwenden IMAGE_GENERATION action
            recaptcha_token, browser_id = await self._get_recaptcha_token(
                project_id,
                action="IMAGE_GENERATION",
                token_id=token_id
            ...
            # 4K/2K UpscaleVerwendenverwendenTimeout, fuerZurueckgeben base64 AnzahlBasierend aufMengegross
            try:
                result = await self._make_request(
                    method="POST",
                    url=url,
                ...
                # Zurueckgeben base64 KodierenCodeBild
    ...
    # ========== Video-Generierung (VerwendenAT) - Zurueckgeben ==========
    ...
    async def generate_video_text(
        ...
        """Text-zu-Video,Zurueckgebentask_id
        ...
        Args:
            ...
            aspect_ratio: VideoSeitenverhaeltnis
            user_paygate_tier: BenutzerLevel
        ...
        # 403/reCAPTCHA RetryLogik - Am meistenvieleRetry3Mal
        max_retries = 3
        last_error = None
        
        for retry_attempt in range(max_retries):
            # jedeMalRetryalleErneutneuAbrufen reCAPTCHA token - VideoVerwenden VIDEO_GENERATION action
            launch_gate_acquired = False
            ...
            if not recaptcha_token:
                ...
                should_retry = await self._handle_missing_recaptcha_token(
                    ...
                    browser_id=browser_id,
                    project_id=project_id,
                    log_prefix="[VIDEO T2V] Generierung",
                )
                if should_retry:
                    continue
                raise last_error
    ...
    async def generate_video_reference_images(
        ...
        """Bild-zu-Video,Zurueckgebentask_id
        ...
        Args:
            ...
            aspect_ratio: VideoSeitenverhaeltnis
            reference_images: BildListe [{"imageUsageType": "IMAGE_USAGE_TYPE_ASSET", "mediaId": "..."}]
            user_paygate_tier: BenutzerLevel

        Returns:
            Gleich generate_video_text
        """
        ...
        for retry_attempt in range(max_retries):
            ...
            if not recaptcha_token:
                ...
                should_retry = await self._handle_missing_recaptcha_token(
                    ...
                    browser_id=browser_id,
                    project_id=project_id,
                    log_prefix="[VIDEO R2V] Generierung",
                )
                if should_retry:
                    continue
                raise last_error
    ...
    async def generate_video_start_end(
        ...
        """empfangenEndframeGenerierungVideo,Zurueckgebentask_id
        ...
        Args:
            ...
            aspect_ratio: VideoSeitenverhaeltnis
            start_media_id: AnfangFramemediaId
            end_media_id: BeendetFramemediaId
            user_paygate_tier: BenutzerLevel
        ...
        for retry_attempt in range(max_retries):
            ...
            if not recaptcha_token:
                ...
                should_retry = await self._handle_missing_recaptcha_token(
                    ...
                    browser_id=browser_id,
                    project_id=project_id,
                    log_prefix="[VIDEO I2V] ErsteEndframeGenerierung",
                )
                if should_retry:
                    continue
                raise last_error
    ...
    async def generate_video_start_image(
        ...
        """NurAnfangsframeGenerierungVideo,Zurueckgebentask_id
        ...
        for retry_attempt in range(max_retries):
            ...
            if not recaptcha_token:
                ...
                should_retry = await self._handle_missing_recaptcha_token(
                    ...
                    browser_id=browser_id,
                    project_id=project_id,
                    log_prefix="[VIDEO I2V] AnfangsframeGenerierung",
                )
                if should_retry:
                    continue
                raise last_error
            ...
            json_data = {
                ...
                "requests": [{
                    ...
                    # Hinweis: KeinhatendImageFeld,NurverwendenAnfangsframe
                    "metadata": {
                        "sceneId": scene_id
                    }
                }]
    ...
    async def upsample_video(
        ...
        """Video-Upscaleauf 4K/1080P, Zurueckgeben task_id

        Args:
            at: Access Token
            project_id: ProjektID
            video_media_id: Video mediaId
            aspect_ratio: VideoSeitenverhaeltnis VIDEO_ASPECT_RATIO_PORTRAIT/LANDSCAPE
            resolution: VIDEO_RESOLUTION_4K Oder VIDEO_RESOLUTION_1080P
            model_key: veo_3_1_upsampler_4k Oder veo_3_1_upsampler_1080p
    ...
    # ========== Aufgaben-Polling (VerwendenAT) ==========

    async def check_video_status(self, at: str, operations: List[Dict]) -> dict:
        """AbfragenAbfragenVideo-GenerierungStatus

        Args:
            at: Access Token
            operations: OperationListe [{"operation": {"name": "task_id"}, "sceneId": "...", "status": "..."}]
        ...
        Returns:
            {
                "operations": [{
                    "operation": {
                        "name": "task_id",
                        "metadata": {...}  # AbgeschlossenbeiEnthaeltVideoInformationen
                    },
        ...
        for retry_attempt in range(max_retries):
            try:
                ...
                if retry_reason and retry_attempt < max_retries - 1:
                    debug_logger.log_warning(
                        f"[VIDEO POLL] StatusabfrageTreffen aufauf{retry_reason}, VorbereitenRetry ({retry_attempt + 2}/{max_retries})..."
                    )
                    await asyncio.sleep(1)
                    continue
                raise
        ...
        if last_error is not None:
            raise last_error
        raise RuntimeError("VideoStatusabfrageFehlgeschlagen")

    # ========== Medien loeschen (VerwendenST) ==========
    ...
    async def _handle_retryable_generation_error(
        ...
        """EinheitlichVerarbeitenGenerierungRouteRetryBestimmenmitCaptcha-LoesungSelbstheilungBenachrichtigung。"""
        error_str = str(error)
        ...
        if is_terminal_attempt:
            debug_logger.log_warning(
                f"{log_prefix}Treffen aufauf{retry_reason}, bereitsErreichenaufAm meistengrossRetryMalAnzahl({max_retries}), MalAnfrageFehlgeschlagenundAusfuehrenSchliessenZurueckempfangen。"
            )
            return False

        debug_logger.log_warning(
            f"{log_prefix}Treffen aufauf{retry_reason}, GeradeinErneutneuAbrufenCaptchaRetry ({retry_attempt + 2}/{max_retries})..."
        )
        await asyncio.sleep(1)
        return True
    ...
    def _get_retry_reason(self, error_str: str) -> Optional[str]:
        """BestimmenistObbenoetigenbrauchenRetry, ZurueckgebenProtokollEinreichenAnzeigeninnerhalb"""
        ...
        if self._is_retryable_network_error(error_str):
            return "Netzwerk/TLSFehler"
        if "recaptcha evaluation failed" in error_lower:
            return "reCAPTCHA ValidierenFehlgeschlagen"
        if "recaptcha" in error_lower:
            return "reCAPTCHA Fehler"
        if any(keyword in error_lower for keyword in [
            ...
            return "500/innerhalbTeilFehler"
        return None
    ...
    async def _notify_browser_captcha_error(
        ...
        """BenachrichtigungBrowser-Captcha-LoesungDienstAusfuehrenFehlgeschlagenSelbstheilung。
        
        Args:
            browser_id: browser ModusVerwendenBrowser ID
            project_id: personal ModusVerwenden project_id
            error_reason: bereitsNormalisierenFehlerGrund
            error_message: OriginalAnfangFehlerText
        """
        if config.captcha_method == "browser":
            ...
            try:
                ...
                debug_logger.log_warning(f"[reCAPTCHA RemoteBrowser] aufMelden error Fehlgeschlagen: {e}")

    async def _notify_browser_captcha_request_finished(self, browser_id: Optional[Union[int, str]] = None):
        """BenachrichtigungHeaded-Browser: UpstreamBild/VideoAnfragebereitsBeendet, kannSchliessenfuerAntwortCaptcha-Browser。"""
        if config.captcha_method == "browser":
            try:
                from .browser_captcha import BrowserCaptchaService
                service = await BrowserCaptchaService.get_instance(self.db)
            ...
            try:
                ...
                debug_logger.log_warning(f"[reCAPTCHA RemoteBrowser] aufMelden finish Fehlgeschlagen: {e}")

    def _generate_session_id(self) -> str:
        """GenerierungsessionId: ;timestamp"""
        return f";{int(time.time() * 1000)}"

    def _generate_scene_id(self) -> str:
        """GenerierungsceneId: UUID"""
    ...
    def _get_remote_browser_service_config(self) -> tuple[str, str, int]:
        ...
        if not base_url:
            raise RuntimeError("remote_browser DienstAdressenichtKonfiguration")
        if not api_key:
            raise RuntimeError("remote_browser API Key nichtKonfiguration")

        if not (base_url.startswith("http://") or base_url.startswith("https://")):
            raise RuntimeError("remote_browser DienstAdresseFormatFehler")
    ...
    async def _sync_json_http_request(
        ...
        try:
            ...
            raise RuntimeError(f"remote_browser AnfrageFehlgeschlagen: {e}") from e
    ...
    async def _call_remote_browser_service(
        ...
        if status_code >= 400:
            ...
            raise RuntimeError(f"remote_browser AnfrageFehlgeschlagen: {detail}")

        if not isinstance(payload, dict):
            raise RuntimeError("remote_browser ZurueckgebenFormatFehler")
    ...
    async def _get_recaptcha_token(
        ...
        """AbrufenreCAPTCHA token - UnterstuetztvieleCaptcha-Methode
        ...
        Args:
            ...
            token_id: wennvorGeschaeft token id(browser ModusunterverwendenFuerlesenabrufen token LevelCaptcha-Proxy)
        
        Returns:
            (token, browser_id) Gruppe。
            - browser Modus: browser_id fuerLokalBrowser ID
            - remote_browser Modus: browser_id fuerRemoteProzess session_id
            - derenModus: browser_id fuer None
        """
        captcha_method = config.captcha_method

        # innerhalbSetzenBrowser-Captcha-Loesung (nodriver)
        if captcha_method == "personal":
            try:
                ...
                #  Docker UmgebungOderFehlenKlarSicherstellenFehler
                error_msg = str(e)
                debug_logger.log_error(f"[reCAPTCHA Personal] {error_msg}")
                print(f"[reCAPTCHA] ❌ innerhalbSetzenBrowser-Captcha-LoesungFehlgeschlagen: {error_msg}")
                self._set_request_fingerprint(None)
                return None, None
            except ImportError as e:
                debug_logger.log_error(f"[reCAPTCHA Personal] ImportierenFehlgeschlagen: {str(e)}")
                print(f"[reCAPTCHA] ❌ nodriver nichtInstallieren, BitteAusfuehren: pip install nodriver")
                self._set_request_fingerprint(None)
                return None, None
            except Exception as e:
                debug_logger.log_error(f"[reCAPTCHA Personal] Fehler: {str(e)}")
                self._set_request_fingerprint(None)
                return None, None
        # hatHeaderBrowser-Captcha-Loesung (playwright)
            ...
            try:
                ...
                print(f"[reCAPTCHA] ❌ hatHeaderBrowser-Captcha-LoesungFehlgeschlagen: {error_msg}")
                self._set_request_fingerprint(None)
                return None, None
            except ImportError as e:
                debug_logger.log_error(f"[reCAPTCHA Browser] ImportierenFehlgeschlagen: {str(e)}")
                print(f"[reCAPTCHA] ❌ playwright nichtInstallieren, BitteAusfuehren: pip install playwright && python -m playwright install chromium")
                self._set_request_fingerprint(None)
                return None, None
            except Exception as e:
                debug_logger.log_error(f"[reCAPTCHA Browser] Fehler: {str(e)}")
                self._set_request_fingerprint(None)
                return None, None
            ...
            try:
                ...
                if not token or not session_id:
                    raise RuntimeError(f"remote_browser ZurueckgebenFehlenwenige token/session_id: {payload}")
                return token, str(session_id)
            except Exception as e:
                debug_logger.log_error(f"[reCAPTCHA RemoteBrowser] Fehler: {str(e)}")
                self._set_request_fingerprint(None)
                return None, None
        # APICaptcha-Dienst
            ...
            async with AsyncSession() as session:
                ...
                for i in range(40):
                    ...
                    if status == 'ready':
                        ...
                        if response:
                            debug_logger.log_info(f"[reCAPTCHA {method}] TokenAbrufenErfolgreich")
                            return response
