...
class FileCache:
    ...
    def _get_request_fingerprint(self) -> Optional[Dict[str, Any]]:
        """lesenabrufenwennvorAnfrageRouteinGebundenBrowser-Fingerabdruck。"""
    ...
    async def _resolve_download_proxy(
        ...
        """BasisBasierend aufMedienTypParsenDownload-ProxyAdresse。"""
        ...
        try:
            # MedienDownload(Bild/Video)BevorzugtVerwendenUnabhaengigUpload/Download-Proxy
    ...
    def _guess_extension(self, url: str, media_type: str) -> str:
        """MengeBeibehaltenOriginalAnfangErweiternErweiternName, nichtWissenbeiFallbackaufStandardWert。"""
    ...
    def _build_download_headers(
        ...
        """StrukturErstellenMedienDownloadAnfrageHeader, BevorzugtWiederverwendenwennvorCaptcha-BrowserZeigenFingerabdruck。"""
        headers = {
            "Accept": (
    ...
    def _write_cached_content(self, file_path: Path, content: bytes):
        """ZuerstschreibenTemporaerbeiDatei, ErneutAtomarErsetzenWechseln, VermeidenParallelitaetlesenaufHalbAbschneidenDatei。"""
    ...
    def _normalize_cache_error(self, error: Exception) -> str:
        """GesamtVerarbeitenCacheFehler, VermeidenwirdBasisEbeneBefehlAusnahmeDirektOffenlegenOffenlegengebenBenutzer。"""
        if isinstance(error, FileNotFoundError):
            missing_name = Path(getattr(error, "filename", "") or "curl").name or "curl"
            return f"MaschinenichtInstallieren {missing_name}"

        message = str(error or "").strip()
        if not message:
            return "nichtWissenFehler"

        if message.startswith("Failed to cache file:"):
            message = message.split(":", 1)[1].strip() or "nichtWissenFehler"
