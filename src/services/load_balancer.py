...
class LoadBalancer:
    ...
    async def _get_token_load(self, token_id: int, for_image_generation: bool, for_video_generation: bool) -> tuple[int, Optional[int]]:
        """Abrufen token wennvorLaden。

        Returns:
            (inflight, remaining)
            remaining fuer None TabelleAnzeigenkeinBegrenzung
        """
    ...
    async def _reserve_slot(self, token_id: int, for_image_generation: bool, for_video_generation: bool) -> bool:
        """Versuchenversuchenfuerwennvor token BelegeneinStueckGenerierungPosition。"""
        if not self.concurrency_manager:
            return True
    ...
    async def select_token(
        ...
        debug_logger.log_info(
            f"[LOAD_BALANCER] oeffnenAnfangAuswahlToken (Bild-Generierung={for_image_generation}, "
            f"Video-Generierung={for_video_generation}, Modell={model}, BelegenPosition={reserve})"
        )

        active_tokens = await self.token_manager.get_active_tokens()
        debug_logger.log_info(f"[LOAD_BALANCER] Abrufenauf {len(active_tokens)} StueckAktivToken")

        if not active_tokens:
            debug_logger.log_info(f"[LOAD_BALANCER] ❌ KeinhatAktivToken")
            return None
        ...
        for token in active_tokens:
            ...
            if model and not supports_model_for_tier(model, normalized_tier):
                filtered_reasons[token.id] = 'KontoLevelnichtAusreichend, benoetigenbrauchen ' + get_paygate_tier_label(required_tier)
                continue
            if for_image_generation:
                if not token.image_enabled:
                    filtered_reasons[token.id] = "Bild-GenerierungbereitsDeaktivieren"
                    continue
                ...
                if (
                    ...
                    filtered_reasons[token.id] = "BildParallelitaetbereitsVoll"
                    continue

            if for_video_generation:
                if not token.video_enabled:
                    filtered_reasons[token.id] = "Video-GenerierungbereitsDeaktivieren"
                    continue
                ...
                if (
                    ...
                    filtered_reasons[token.id] = "VideoParallelitaetbereitsVoll"
                    continue
        ...
        if filtered_reasons:
            debug_logger.log_info(f"[LOAD_BALANCER] bereitsToken:")
            for token_id, reason in filtered_reasons.items():
                debug_logger.log_info(f"[LOAD_BALANCER]   - Token {token_id}: {reason}")

        if not available_tokens:
            debug_logger.log_info(f"[LOAD_BALANCER] ❌ KeinhatVerfuegbarToken (Bild-Generierung={for_image_generation}, Video-Generierung={for_video_generation})")
            return None

        # Am meisten in-flight Bevorzugt; hatParallelitaets-Obergrenzebei, VerbleibendPositionviele token Bevorzugt; Am meistennachMaschineVerteilen
        ...
        debug_logger.log_info("[LOAD_BALANCER] KandidatAuswahlTokenLaden:")
        ...
        # NurfuerKandidatAuswahlListeinwahrGeradeVersuchenversuchenauf token machen AT Validierung, VermeidenjedeMalAnfragewirdallehat token alleScanneneinDurchlauf
        for item in available_tokens:
            token = item["token"]
            token_id = token.id
            ...
            if not token:
                debug_logger.log_info(f"[LOAD_BALANCER] Ueberspringen Token {token_id}: ATUngueltigOderbereitsAbgelaufen")
                continue

            if reserve and not await self._reserve_slot(token.id, for_image_generation, for_video_generation):
                debug_logger.log_info(f"[LOAD_BALANCER] Ueberspringen Token {token.id}: BelegenPositionFehlgeschlagen")
                continue
            ...
            debug_logger.log_info(
                f"[LOAD_BALANCER] ✅ bereitsAuswahlToken {token.id} ({token.email}) - "
                f"Guthaben: {token.credits}, inflight={item['inflight']}"
            )
            return token

        debug_logger.log_info(f"[LOAD_BALANCER] ❌ KandidatAuswahlTokenAllenichtVerfuegbar (Bild-Generierung={for_image_generation}, Video-Generierung={for_video_generation})")
        return None
    ...
    async def get_unavailable_reason(
        ...
        *,
        ...
        """gebenKlarSicherstellen"keinVerfuegbarKonto"Grund, BevorzugtverwendenFuerUnterscheidenRate/tier DateiPositionEinreichenAnzeigen。"""
        active_tokens = await self.token_manager.get_active_tokens()
        if not active_tokens:
            return None
        ...
        if model and not supported_tokens:
            ...
            return f"wennvorModellbenoetigenbrauchen {tier_label} Konto, aberKeinhatVerfuegbar {tier_label} Konto: {model}"
        ...
        if supported_tokens and not capability_tokens:
            if for_image_generation:
                return "wennvorhatEntsprechenZusammenDateiPositionKonto, aberBild-GenerierungFunktionkannbereitsalleTeilDeaktivieren。"
            if for_video_generation:
                return "wennvorhatEntsprechenZusammenDateiPositionKonto, aberVideo-GenerierungFunktionkannbereitsalleTeilDeaktivieren。"
