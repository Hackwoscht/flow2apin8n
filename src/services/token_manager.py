...
class TokenManager:
    ...
    # ========== TokenHinzufuegen (UnterstuetztProjectErstellen) ==========
    ...
    async def update_token(
        ...
        """Update token (UnterstuetztAendernproject_idUndproject_name)

        wennBenutzerBearbeitenSpeicherntokenbei, wieErgebnistokennichtAbgelaufen, AutomatischBereinigenleer429Deaktivierungsstatus
        """
        update_fields = {}
        ...
        # PruefentokenistOb429wurdeDeaktivieren, wieErgebnisistundnichtAbgelaufen, dannBereinigenleer429Status
        token = await self.db.get_token(token_id)
        if token and token.ban_reason == "429_rate_limit":
            # PruefentokenistObAbgelaufen
            is_expired = False
            if token.at_expires:
            ...
            # wieErgebnisnichtAbgelaufen, Bereinigenleer429Deaktivierungsstatus
            if not is_expired:
                debug_logger.log_info(f"[UPDATE_TOKEN] Token {token_id} BearbeitenSpeichern, Bereinigenleer429Deaktivierungsstatus")
    ...
    # ========== ATAuto-AktualisierungLogik (Kern) ==========

    def _should_refresh_at(self, token: Token) -> bool:
        """BasisBasierend aufwennvor token SnapshotBestimmenistObbenoetigenbrauchenAktualisieren AT。"""
        if not token.at:
            debug_logger.log_info(f"[AT_CHECK] Token {token.id}: ATnichtspeichernin,benoetigenbrauchenAktualisieren")
            return True

        if not token.at_expires:
            debug_logger.log_info(f"[AT_CHECK] Token {token.id}: ATAblaufzeitnichtWissen,VersuchenversuchenAktualisieren")
            return True
        ...
        if time_until_expiry.total_seconds() < 3600:
            debug_logger.log_info(
                f"[AT_CHECK] Token {token.id}: ATSofortwirdAbgelaufen "
                f"(Verbleibend {time_until_expiry.total_seconds():.0f} Sekunden),benoetigenbrauchenAktualisieren"
            )
            return True
    ...
    async def ensure_valid_token(self, token: Optional[Token]) -> Optional[Token]:
        """Sicherstellen token  AT Verfuegbar, undinMussbrauchenbeiZurueckgebenAktualisierennachAm meistenneuObjekt。"""
        if not token:
            return None
    ...
    async def is_at_valid(self, token_id: int, token: Optional[Token] = None) -> bool:
        """PruefenATistObGueltig,wieErgebnisUngueltigOderSofortwirdAbgelaufendannAuto-Aktualisierung
    ...
    async def _do_refresh_at(self, token_id: int, st: str) -> bool:
        """Ausfuehren AT AktualisierenKernLogik
        ...
        """
        try:
            debug_logger.log_info(f"[AT_REFRESH] Token {token_id}: oeffnenAnfangAktualisierenAT...")
            ...
            debug_logger.log_info(f"[AT_REFRESH] Token {token_id}: ATAktualisierenErfolgreich")
            debug_logger.log_info(f"  - neuAblaufzeit: {new_at_expires}")

            # Validieren AT Gueltig: Durch get_credits Test
            try:
                ...
                debug_logger.log_info(f"[AT_REFRESH] Token {token_id}: AT ValidierenErfolgreich(Guthaben: {credits_result.get('credits', 0)})")
                return True
            except Exception as verify_err:
                # AT ValidierenFehlgeschlagen(kannkannZurueckgeben 401), Erklaerung ST bereitsAbgelaufen
                error_msg = str(verify_err)
                if "401" in error_msg or "UNAUTHENTICATED" in error_msg:
                    debug_logger.log_warning(f"[AT_REFRESH] Token {token_id}: AT ValidierenFehlgeschlagen (401), ST kannkannbereitsAbgelaufen")
                    return False
                else:
                    # derenFehler(wieNetzwerkProblem), Immer nochBetrachtenfuerErfolgreich
                    debug_logger.log_warning(f"[AT_REFRESH] Token {token_id}: AT ValidierenbeisendenGenerierenNicht-AuthentifizierungFehler: {error_msg}")
                    return True

        except Exception as e:
            debug_logger.log_error(f"[AT_REFRESH] Token {token_id}: ATAktualisierenFehlgeschlagen - {str(e)}")
            return False

    async def _try_refresh_st(self, token_id: int, token) -> Optional[str]:
        """VersuchenversuchenDurchBrowserAktualisieren Session Token

        VerwendenResident tab Abrufenneu __Secure-next-auth.session-token
        ...
        Returns:
            neu ST String, wieErgebnisFehlgeschlagenZurueckgeben None
        """
        try:
            from ..core.config import config

            # Nurin personal ModusunterUnterstuetzt ST Auto-Aktualisierung
            if config.captcha_method != "personal":
                debug_logger.log_info(f"[ST_REFRESH] Nicht- personal Modus, Ueberspringen ST Auto-Aktualisierung")
                return None

            if not token.current_project_id:
                debug_logger.log_warning(f"[ST_REFRESH] Token {token_id} Keinhat project_id, keinMethodeAktualisieren ST")
                return None

            debug_logger.log_info(f"[ST_REFRESH] Token {token_id}: VersuchenversuchenDurchBrowserAktualisieren ST...")
            ...
            if new_st and new_st != token.st:
                # AktualisierenDatenbankin ST
                await self.db.update_token(token_id, st=new_st)
                debug_logger.log_info(f"[ST_REFRESH] Token {token_id}: ST bereitsAutomatisch aktualisieren")
                return new_st
            elif new_st == token.st:
                debug_logger.log_warning(f"[ST_REFRESH] Token {token_id}: Abrufenauf ST mitOriginal ST Gleich, kannkannAnmeldenbereitsUngueltig")
                return None
            else:
                debug_logger.log_warning(f"[ST_REFRESH] Token {token_id}: keinMethodeAbrufenneu ST")
                return None

        except Exception as e:
            debug_logger.log_error(f"[ST_REFRESH] Token {token_id}: Aktualisieren ST Fehlgeschlagen - {str(e)}")
            return None
    ...
    async def ban_token_for_429(self, token_id: int):
        """429FehlerSofortSofortDeaktivierentoken

        Args:
            token_id: Token ID
        """
        debug_logger.log_warning(f"[429_BAN] DeaktivierenToken {token_id} (Grund: 429 Rate Limit)")
        await self.db.update_token(
            token_id,
            is_active=False,
    ...
    async def auto_unban_429_tokens(self):
        """Automatisch entsperren429wurdeDeaktivierentoken

        Regel:
        - AbstandDeaktivierungszeitpunkt12StundennachAutomatisch entsperren
        - NurEntsperrennichtAbgelaufentoken
        - NurEntsperren429wurdeDeaktivierentoken
        """
        all_tokens = await self.db.get_all_tokens()
        now = datetime.now(timezone.utc)

        for token in all_tokens:
            # UeberspringenNicht-429Deaktivierentoken
            if token.ban_reason != "429_rate_limit":
                continue

            # UeberspringennichtDeaktivierentoken
            if token.is_active:
                continue

            # UeberspringenKeinhatDeaktivierungszeitpunkttoken
            if not token.banned_at:
                continue

            # PruefentokenistObbereitsAbgelaufen
            if token.at_expires:
                # SicherstellenbeiBereichKonsistent
                if token.at_expires.tzinfo is None:
                    at_expires_aware = token.at_expires.replace(tzinfo=timezone.utc)
                else:
                    at_expires_aware = token.at_expires

                # wieErgebnisbereitsAbgelaufen, Ueberspringen
                if at_expires_aware <= now:
                    debug_logger.log_info(f"[AUTO_UNBAN] Token {token.id} bereitsAbgelaufen, UeberspringenEntsperren")
                    continue

            # Sicherstellenbanned_atbeiBereichKonsistent
            ...
            # PruefenistObbereits12Stunden
            time_since_ban = now - banned_at_aware
            if time_since_ban.total_seconds() >= 12 * 3600:  # 12Stunden
                debug_logger.log_info(
                    f"[AUTO_UNBAN] EntsperrenToken {token.id} (Deaktivierungszeitpunkt: {banned_at_aware}, "
                    f"bereits {time_since_ban.total_seconds() / 3600:.1f} Stunden)"
                )
                await self.db.update_token(
                    token.id,
                    is_active=True,
                ...
                # ZuruecksetzenFehlerzaehlung
                await self.db.reset_error_count(token.id)

    # ========== Guthaben-Aktualisierung ==========
