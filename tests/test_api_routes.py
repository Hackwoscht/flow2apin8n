...
def test_openai_route_returns_handler_error_status(client, fake_handler):
    fake_handler.non_stream_chunks = [
        json.dumps(
            {
                "error": {
                    "message": "KeinhatVerfuegbarTokenFortschreitenAusfuehrenBild-Generierung",
                    "status_code": 503,
                }
            }
        )
    ...
    assert response.json()["error"]["message"] == "KeinhatVerfuegbarTokenFortschreitenAusfuehrenBild-Generierung"
